...
import { IProcessTask, ProcessTask } from '../model/process-task.model';
...
export class AbstractEntityStore<T> {
  ...
  protected _readItem(action: Action): Observable<any> {
    const item: T = action.payload;
    const prevState = this.stateSubject.getValue();
    return combineLatest([
      this.itemService.find(this.id(item)).pipe(map(res => res.body)),
      iif(() => this.useTask, this.itemService.getTasks(this.id(item)).pipe(map(res => res.body)), of([])),
    ]).pipe(
      map(([currItem, currTask]) => {
        const newState = { ...prevState, item: currItem, tasks: currTask };
        return newState;
      }),
      mergeMap(newState => this.loadRelatedEntityEffect(newState)),
      map(newState => {
        const newItems = this.updateItemsFrom(newState.items.slice(0), newState.item, (r, s) => this.isEqualId(r, s));
        return { ...newState, error: {}, items: newItems };
      })
    );
  }

  protected _pushItem(action: Action): Observable<any> {
    const item: T = action.payload;
    const prevState = this.stateSubject.getValue();
    return combineLatest([
      of(item),
  ...
  protected _updateItem(action: Action): Observable<any> {
    ...
    return this.saveItem$(prevState).pipe(
      map(state => {
        const newItems = this.updateItemsFrom(state.items.slice(0), state.item, (r, s) => this.isEqualId(r, s));
        // new State
  ...
  protected _deleteItem(action: Action): Observable<any> {
    ...
    return this.itemService.delete(this.id(item)).pipe(
      map(() => prevState),
      map(state => {
        const newItems = this.removeItemBy(state.items.slice(0), state, (r, s) => this.isEqualId(r, s));
  ...
  protected _processTask(action: Action): Observable<any> {
    const task: IProcessTask = action.payload;
    const prevState = this.stateSubject.getValue();
    return this.itemService.processTask(task).pipe(
      map(res => res.body),
      map(taskResult => {
        const msg = taskResult.message;
        if (msg) {
          let sevr = 'info';
          let sumr = 'Task Summary';
          if (taskResult.statusResult === 99) {
            sevr = 'error';
            sumr = 'Error Information';
          }
          this.displayMessage(sevr, sumr, msg);
        }
      }),
      map(() => prevState),
      mergeMap(newState => this.itemService.find(this.id(newState.item)).pipe(map(res => ({ ...newState, item: res.body })))),
      mergeMap(newState => this.itemService.getTasks(this.id(newState.item)).pipe(map(res => ({ ...newState, tasks: res.body }))))
    );
  }

  protected reducer(action: Action): Observable<any> {
    switch (action.type) {
      case CREATE_ITEM: {
        return this._updateItem(action);
      }
      case READ_ITEM: {
        return this._readItem(action);
      }
      case PUSH_ITEM: {
        return this._pushItem(action);
      }
      case UPDATE_ITEM: {
        return this._updateItem(action);
      }
      case DELETE_ITEM: {
        return this._deleteItem(action);
      }
      case PROCESS_TASK: {
        return this._processTask(action);
      }
      case ASSIGN_ITEMS: {
        const prevState = this.stateSubject.getValue();
        const newState = { ...prevState, items: action.payload };
        this.stateSubject.next(newState);
        return of(newState);
      }
    }
  ...
  updateItemsFrom(source: T[], item: T, fn: (res: T, val: any) => boolean): T[] {
  ...
  dispatch(action: Action, fn?: (s: any) => void) {
    this.reducer(action).subscribe((state: any) => {

...
@Component({
  selector: 'jhi-billing-view',
  templateUrl: './billing-view.component.html',
})
export class BillingViewComponent extends AbstractEntityBaseViewComponent<IBilling> implements OnChanges {
  @Input() id: number;
  ...
  constructor(
    ...
    super(billingService, messageService, elementRef, dataUtils, account, eventManager);
    this.settlementSelect = new Settlement();
    this.item = new Billing();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new Billing();
        this.billingService.find(this.id).subscribe(result => {
  ...
  prepareView() {
    if (this.billing.settlementId) {
      this.settlementService.find(this.billing.settlementId).subscribe(

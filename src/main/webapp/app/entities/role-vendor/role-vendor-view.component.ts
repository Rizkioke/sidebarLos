...
@Component({
  selector: 'jhi-role-vendor-view',
  templateUrl: './role-vendor-view.component.html',
})
export class RoleVendorViewComponent extends AbstractEntityBaseViewComponent<IRoleVendor> implements OnChanges {
  @Input() id: number;
  ...
  constructor(
    ...
    super(roleVendorService, messageService, elementRef, dataUtils, account, eventManager);
    this.item = new RoleVendor();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new RoleVendor();
        this.roleVendorService.find(this.id).subscribe(result => {
  ...
  set roleVendor(roleVendor: IRoleVendor) {
    this.item = roleVendor;
  }

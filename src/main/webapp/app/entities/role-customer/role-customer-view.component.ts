...
@Component({
  selector: 'jhi-role-customer-view',
  templateUrl: './role-customer-view.component.html',
})
export class RoleCustomerViewComponent extends AbstractEntityBaseViewComponent<IRoleCustomer> implements OnChanges {
  @Input() id: number;
  ...
  constructor(
    ...
    super(roleCustomerService, messageService, elementRef, dataUtils, account, eventManager);
    this.item = new RoleCustomer();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new RoleCustomer();
        this.roleCustomerService.find(this.id).subscribe(result => {
  ...
  set roleCustomer(roleCustomer: IRoleCustomer) {
    this.item = roleCustomer;
  }

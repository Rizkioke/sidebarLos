...
@Component({
  selector: 'jhi-fin-account-view',
  templateUrl: './fin-account-view.component.html',
})
export class FinAccountViewComponent extends AbstractEntityBaseViewComponent<IFinAccount> implements OnChanges {
  @Input() id: number;
  ...
  constructor(
    ...
    super(finAccountService, messageService, elementRef, dataUtils, account, eventManager);
    this.item = new FinAccount();
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['id']) {
      ...
      if (this.id) {
        this.item = new FinAccount();
        this.finAccountService.find(this.id).subscribe(result => {
  ...
  set finAccount(finAccount: IFinAccount) {
    this.item = finAccount;
  }

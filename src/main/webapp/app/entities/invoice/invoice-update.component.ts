...
export class InvoiceUpdateComponent extends AbstractEntityUpdateComponent<IInvoice> {
  ...
  constructor(
    ...
    super(dataUtils, invoiceService, elementRef, confirmationService, toastService, activatedRoute);
    this.useTask = true;
    this.settlementSelect = new Settlement();
    this.listChangeEventName = 'invoiceListModification';
  }

  protected initialState(): any {
    return { item: new Invoice(), tasks: [], id: undefined };
  }
  ...
  initialize() {
    ...
    this.billingTypeService.loadCacheAll()
        .subscribe((res: IBillingType[]) => (this.billingtypes = res.filter(x => x.parentId === 'SALES_INVOICE' || x.id === 'SALES_INVOICE') || []));

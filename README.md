...
To build the final jar and optimize the losgw application for production, run:
...
For running FE without running backend, open file proxy.conf.js on webpack folder then change target from localhost to target IP,

```diff
- DO NOT PUSH THAT FILE TO GITLAB REPOSITORY IF YOU CHANGE IT TO ANOTHER TARGET EXCEPT LOCALHOST
```
